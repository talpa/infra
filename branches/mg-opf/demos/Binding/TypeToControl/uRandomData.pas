...
function RandomCompanySuffixes: string;
...
const
  ...
  Countries: array[0..6] of string = ('Brasil', 'Alemanha', 'Japão',
    'Inglaterra', 'Peru', 'Mexico', 'Iraque');
  ...
  States: array[0..29] of string = (
    'PR', 'SC', 'RJ', 'RS', 'SP', 'MT', 'CA', 'PE', 'PM', 'PI',
    'TE', 'TP', 'TO', 'ES', 'BA', 'MG', 'MS', 'AR', 'AA', 'AL',
    'DC', 'FR', 'QA', 'QI', 'SS', 'HM', 'LT', 'YW', 'YI', 'TX'
    );
...
begin
  Randomize;
  case Random(2) of
    0: Result := RandomFullName(gnMale);
    1: Result := RandomFullName(gnFemale);
  end;
end;
...
function RandomCountry: string;
begin
  Result := RandomStr(Countries);
end;

function RandomState: string;
begin
  Result := RandomStr(States);
end;

function RandomCompanySuffixes: string;
begin
  Result := RandomStr(CompanySuffixes);
end;
