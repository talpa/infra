...
type
  ...
  protected
    ...
    procedure Next;
  public
    constructor Create(const List: IPresenterList);
  end;
...
begin
  ...
  if Model.Selection.Count = 0 then
    ...
    (Model.Selection.Items[0] as IInfraString).AsString :=
      (Renderer.TypeConverter.ConvertFromRightToLeft(
...
begin
  ...
  begin
    if Assigned(Model) and Assigned(Model.Value) then
      FCustomEdit.Text :=
        (Renderer.TypeConverter.ConvertFromLeftToRight(Model.Value,
          Renderer.Format) as IInfraString).AsString
    else
      FCustomEdit.Text := '';
  end;
...
begin
  ...
  begin
    if Assigned(Model) and Assigned(Model.Value) then
      FCustomLabel.Caption := (Renderer.TypeConverter.ConvertFromLeftToRight(
...
begin
  inherited;
  if Assigned(VCLObject) then
    FRadioButton.Checked := (Renderer.TypeConverter.ConvertFromLeftToRight(
...
begin
  inherited;
  if Assigned(VCLObject) then
    Model.Value.Assign(Renderer.TypeConverter.ConvertFromRightToLeft(
...
begin
  inherited;
  if Assigned(VCLObject) then
    FCustomCheckBox.Checked := (Renderer.TypeConverter.ConvertFromLeftToRight(
      Model.Value, Renderer.Format) as IInfraBoolean).AsBoolean;
end;
...
var
  vSrc: IInfraList;
  vFeature: IInfraFeature;
begin
  Result :=
    (Supports(Event.Source, IInfraFeature, vFeature) and
      (ListItemView.FeatureName = vFeature.Name))
    or
...
begin
  ...
  if Assigned(Renderer) and
    ...
    Result := (Renderer.Format as IFormatFeature).FeatureName
  else
...
begin
  with (ApplicationContext as IMemoryManagedObject).InjectedInterfaces do
    Add(IMVPMapperService, TMVPMapperService.Create as IMVPMapperService);
end;
